...
- 每次运行先写完整 `run-s6-S6-review.md`，再生成共同信封 `schema_version=1.3.0`，最后生成已填充的 `run-s6-S6-report.html`；
  Markdown 各章节必须通过 `document_artifact.section_mappings` 与 JSON 实质内容对应，不能只保留教学提纲。
  页面专门展示当前节点、教学内容、正反例、活动、渐进提示、既有交互轨迹与候选证据。
- 页面只允许查看、切换和展开既有数据，不显示装载器、Schema/模板/Markdown/HTML 生成状态，也不接受新作答或产生事件。
- 页面是唯一可视化交付，完全由内嵌 JSON 驱动；本次运行不调用制图工具，也不生成或引用任何独立图片资产。
- 学习者未作答时仍必须生成诊断页面，并直接显示缺失输入和恢复顺序。生成或验证失败时使用
