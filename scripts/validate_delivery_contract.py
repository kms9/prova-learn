...
LEGACY_S1_FILES = ["INDEX.md","sources.jsonl","evidence_table.md","coverage.md","capabilities.jsonl","terminology.jsonl","goal-contract.md","research-brief.md","confirmation.md","g1-evaluation.md","verification.md"]
LEGACY_S2_FILES = ["s2/INDEX.md","s2/research-log.md","s2/sources.jsonl","s2/coverage.md","s2/evidence-items.jsonl","s2/research-answers.md","s2/answer-validation.md","s2/g2-evaluation.md","s2/verification.md"]
...
def validate_jsonl(path: Path, errors: list[str], immutable: bool = False) -> None:
    ...
    for line_no, line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        ...
        if not isinstance(item, dict):
            ...
            continue
        if immutable:
            require(item.get("immutable") is True, f"event must be immutable {path}:{line_no}", errors)
            event_id = item.get("event_id")
            require(isinstance(event_id, str) and event_id, f"event_id missing {path}:{line_no}", errors)
            if isinstance(event_id, str):
                require(event_id not in seen, f"duplicate event_id {event_id} in {path}", errors)
                seen.add(event_id)

def validate_manifest() -> tuple[dict[str, Any], list[str]]:
    errors: list[str] = []
    manifest = load_json(MANIFEST_PATH)
    require(manifest.get("manifest_version") == "2.1.0", "manifest_version must be 2.1.0", errors)
    workspace = manifest.get("workspace")
    require(isinstance(workspace, dict), "manifest.workspace must be object", errors)
    if isinstance(workspace, dict):
        require(workspace.get("delivery_type") == "project_llm_wiki", "workspace delivery_type invalid", errors)
        for key in ("contract","standalone_contract","state_schema","event_schema"):
            rel = workspace.get(key)
            require(isinstance(rel, str) and (REPO / rel).is_file(), f"workspace {key} missing: {rel}", errors)
        ...
        require(set(append_only).issubset(required), "append-only conversation files must be required", errors)
    common = manifest.get("common_stage_files")
    require(isinstance(common, list) and common, "common_stage_files missing", errors)
    ...
    profiles = load_json(profiles_path).get("stages", {}) if profiles_path.is_file() else {}
    seen_dirs: list[str] = []
    for stage_id, cfg in stages.items():
        ...
        require(profile.get("stage_dir") == cfg.get("stage_dir"), f"{prefix} profile stage_dir mismatch", errors)
        require(profile.get("prerequisites") == cfg.get("prerequisites"), f"{prefix} profile prerequisites mismatch", errors)
        require(cfg.get("delivery_type") == "project_wiki_stage", f"{prefix} wrong delivery_type", errors)
        stage_dir = cfg.get("stage_dir")
        require(isinstance(stage_dir, str) and stage_dir.startswith("stages/"), f"{prefix} invalid stage_dir", errors)
        ...
        if skill_md.is_file():
            ...
            require(actual == cfg.get("skill_name"), f"{prefix} skill name drift: {actual}", errors)
        eval_path = skill_dir / "evals/evals.json"
        require(eval_path.is_file(), f"{prefix} missing evals", errors)
        if eval_path.is_file():
            evals = load_json(eval_path)
            require(evals.get("skill_name") in eval_aliases(stage_id, cfg), f"{prefix} eval skill_name drift", errors)
        specific = cfg.get("required_stage_files")
        require(isinstance(specific, list) and specific, f"{prefix} required_stage_files missing", errors)
        if isinstance(common, list) and isinstance(specific, list):
            ...
            require(cfg.get("entrypoint") in common + specific, f"{prefix} entrypoint not required", errors)
        ...
        if isinstance(prerequisites, list):
            for req in prerequisites:
                ...
                if req_stage in EXPECTED_STAGES:
                    require(int(req_stage[1:]) < int(stage_id[1:]), f"{prefix} prerequisite must be earlier", errors)
                    require(req_stage not in seen_req, f"{prefix} duplicate prerequisite {req_stage}", errors)
                    seen_req.add(req_stage)
                    require(req.get("gate") == f"G{req_stage[1:]}", f"{prefix} prerequisite gate mismatch", errors)
                    require(req.get("state") == "completed", f"{prefix} prerequisite state must be completed", errors)
        if stage_id == "S1":
            require(prerequisites == [], "S1 must have no prerequisites", errors)
        else:
            require("prerequisite-check.md" in (specific or []), f"{prefix} prerequisite-check.md required", errors)
        if stage_id in {"S3","S4","S5","S6","S7"}:
            require(bool(cfg.get("compatibility_exports")), f"{prefix} compatibility exports missing", errors)
    require(unique(seen_dirs), "stage directories must be unique", errors)
    for path in (REPO / ".agents/AGENTS.md", REPO / "workspace/AGENTS.md", REPO / "scripts/record_prerequisite_block.py", REPO / ".agents/stage-runtime-contract.md"):
...
def validate_project(manifest: dict[str, Any], project: Path) -> list[str]:
    ...
    require(isinstance(state.get("project_revision"), int) and state["project_revision"] >= 1, "project_revision invalid", errors)
    require(state.get("current_stage") in EXPECTED_STAGES, "current_stage invalid", errors)
    require(state.get("route_to") in EXPECTED_STAGES + ["complete"], "route_to invalid", errors)
    ...
    for stage_id, cfg in manifest["stages"].items():
        ...
        if stage_state == "completed":
            ...
            for req in cfg.get("prerequisites", []):
                ...
                require(req_status.get("state") == "completed", f"{stage_id}: {req['stage']} not completed", errors)
                require(req_status.get("gate") == "pass", f"{stage_id}: {req['stage']} gate not pass", errors)
                entry = req_status.get("entrypoint")
                require(isinstance(entry, str) and (project / entry).is_file(), f"{stage_id}: prerequisite entry missing", errors)
        if stage_state == "blocked" and stage_id != "S1":
            require((stage_dir / "prerequisite-check.md").is_file(), f"{stage_id} blocked without prerequisite check", errors)
...
def main() -> int:
    ...
    if args.project_dir:
        ...
        errors.extend(validate_project(manifest, project) if project.is_dir() else [f"project directory does not exist: {project}"])
    if args.legacy_run_dir:
        run = args.legacy_run_dir.resolve()
        errors.extend(validate_legacy(run) if run.is_dir() else [f"legacy run directory does not exist: {run}"])
    if errors:
